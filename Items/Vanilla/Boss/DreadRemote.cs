...
{
	public class DreadRemote : ModItem
	{
		// public bool limitBroken = false;

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Dread Remote");
			Tooltip.SetDefault("[c/909090:Unconsumable]\n[c/909090:Summons:] [c/A0A0A0:Skeletron Prime]\n[c/606060:Usable at Night]");
			ItemID.Sets.SortingPriorityBossSpawns[item.type] = 1025;
		}
		...
		public override bool CanUseItem(Player player)
		{
			...
			// If its Nighttime, and no Skeletron Prime is alive
			else
		...
		public override bool UseItem(Player player)
		{
			...
			Main.NewText(string.Format("[i/s1:2107] [c/A0A0A0:Skeletron Prime] [c/909090:has been reactivated.]"));
			NPC.NewNPC((int)player.position.X + Main.rand.Next(-800, 800), (int)player.position.Y - 1200, NPCID.SkeletronPrime);
		...
		public override void AddRecipes()
		{
			...
			if (ModContent.GetInstance<MainConfig>().RecipeMode == "Custom Recipes")
			{
				...
				recipe.AddRecipeGroup("MomlobInfBoss:AdamantiteBars", 25);
			...
			{
				ModRecipe recipe = new ModRecipe(mod);
				recipe.AddIngredient(ItemID.MechanicalSkull, ModContent.GetInstance<MainConfig>().RecipeMultiplier);
