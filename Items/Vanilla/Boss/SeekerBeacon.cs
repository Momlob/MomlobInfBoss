...
{
	public class SeekerBeacon : ModItem
	{
		// public bool limitBroken = false;

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Seeker Beacon");
			Tooltip.SetDefault("[c/909090:Unconsumable]\n[c/909090:Summons:] [c/A0A0A0:The Twins]\n[c/606060:Usable at Night]");
			ItemID.Sets.SortingPriorityBossSpawns[item.type] = 1023;
		}
		...
		public override bool CanUseItem(Player player)
		{
			...
			// If its Nighttime, and neither Retinazer nor Spazmatism is alive
			else
		...
		{
			// Spawn Retinazer and Spazmatism
			Main.NewText(string.Format("[i/s1:2106] [c/A0A0A0:The Twins] [c/909090:have been reactivated.]"));
			NPC.NewNPC((int)player.position.X + player.direction * 1200, (int)player.position.Y - Main.rand.Next(200, 400), NPCID.Retinazer);
			NPC.NewNPC((int)player.position.X + player.direction * 1200, (int)player.position.Y - Main.rand.Next(200, 400), NPCID.Spazmatism);
		...
		public override void AddRecipes()
		{
			...
			if (ModContent.GetInstance<MainConfig>().RecipeMode == "Custom Recipes")
			{
				...
				recipe.AddRecipeGroup("MomlobInfBoss:CobaltBars", 25);
				...
				recipe.AddRecipeGroup("MomlobInfBoss:EvilHardIngreds", 10);
			...
			if (ModContent.GetInstance<MainConfig>().RecipeMode == "Vanilla Recipes")
			{
				...
				recipe.AddRecipeGroup("MomlobInfBoss:Lens", 3);
			...
			{
				ModRecipe recipe = new ModRecipe(mod);
				recipe.AddIngredient(ItemID.MechanicalEye, ModContent.GetInstance<MainConfig>().RecipeMultiplier);
